...
let main = foreign "Dyn_nat.Main" (pclock @-> mclock @-> time @-> network @-> network
                                   @-> stackv4 @-> conduit @-> resolver @-> job)

let manage_config = Ipaddr.V4.({
    address = of_string_exn "10.0.0.3";
    network = Prefix.of_string_exn "10.0.0.0/24";
    gateway = Some (of_string_exn "10.0.0.1");
  })
...
let stack = generic_stackv4 ~config:manage_config (netif "0")
...
let () =
  ...
  register ~packages ~libraries ~keys "pih-bridge" [
    ...
    $ stack $ conduit_impl $ resolver_impl
  ]
